...
The ECAL material has a radiation length~\ref{eqn:radiation_length} of 0.89 cm and Moli\`ere radius ($\mathrm{R_{M} \simeq 2.19~cm}$).
...
The showering process can be simply described as a combination of photon emission, and electron-positron pair production.
...
The CMS ECAL has a depth of \~25$X_{0}$ at small (0-1.3) and large (1.6-3.0) values of $\eta$.
...
The main goal when using the PF algorithm is to account for all energy deposits measured in the detector.
The way this is done is by clustering energy deposits measured across all subdetectors into separate PF candidates, and classifying these cadidates.
...
Multiple pairs of protons can interact at each bunch crossing, and the number of interactions is stochastic but depends on the beam density.
...
Z bosons decay leptonically to $e\bar{e}$, $\mu\bar{\mu}$, and $\tau\bar{\tau}$~pairs approximately 10\% of the time.
The rest of the time they decay to neutrinos, or to quark-antiquark pairs.
This analysis focuses on decays to $e\bar{e}$ and $\mu\bar{\mu}$ because identifying $\tau$s is very complicated in CMS, and their resolution is not as good as electrons and muons.
...
The depth of the ECAL is \textasciitilde25 radiation lengths which means electrons and photons lose almost all of their energy in the ECAL.
