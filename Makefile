...
includes.tex		?= $(shell ls *.tex | grep -v thesis.tex)
#includes.tex.sh	?=
#includes.tex.pl	?=
#includes.tex.py	?=
#includes.rst		?=
...
make-ps		= \
	$(DVIPS) -z -Ppdf -o '$2' $(if $(filter-out BEAMER,$4),-t$(firstword $4),-tletter) '$1' \
		$(if $(filter BEAMER,$4),| $(enlarge_beamer)) > $3 2>&1
...
%.pdf: %.ps %.embed.make
	$(QUIET)$(call echo-build,$<,$@)
	$(QUIET)$(call make-pdf,$<,$@,$@.log,$*.embed.make); \
	if [ x"$$?" = x"0" ]; then \
	    $(if $(VERBOSE),$(CAT) $@.log,:); \
	    $(TOUCH) '$@'; \
	    $(call copy-with-logging,$@,$(BINARY_TARGET_DIR)); \
...
%.$(build_target_extension): %.bbl %.aux %.$(build_target_extension).1st.make
	...
	@$(CP) $@ $(@:.pdf=.rwk.pdf) 
	@$(CP) $(@:.pdf=.synctex.gz) $(@:.pdf=.rwk.synctex.gz)
...
%.$(build_target_extension).1st.make %.d %.aux %.aux.make %.fls: %.tex
	...
	$(EGREP) -q "# MISSING" $*.d && $(SLEEP) 1 && $(ECHO) MWLMWLMWL $(RM) $*.pdf; \
...
quick: thesis.tex */*.tex
	$(QUIET)pdflatex -file-line-error -halt-on-error thesis.tex  2>&1 | tail -n 30
	$(QUIET)cp thesis.pdf thesis.rwk.pdf
	$(QUIET)cp thesis.synctex.gz thesis.rwk.synctex.gz
