...
\begin{equation}
	N_{\mathrm{LL}}^{\mathrm{SR}}(\HT,N_j,N_b,\mttwomath) = N_{\mathrm{1L}}^{\mathrm{CR}}(\HT,N_j,N_b,\mttwomath) \cdot R_{\mathrm{MC}}^{0l/1l}(\HT,N_j,N_b,\mttwomath) \cdot k(\mttwomath)
...
\begin{table}
	\centering
	\begin{tabular}[]{l c r}
		\fm{Table of lostlep hybrid extrapolation points points} 
	\end{tabular}
...
Several sources of systematic uncertainty are assessed for the lost lepton estimate, including those associated with the topological transfer factor and the \mttwo shape modeling in simulation. The full list of systematic uncertainties is as follows:
\begin{itemize}
